...
$count2=1;
	...
	while($month_counter <= 12){
			...
			$monthtable="SELECT * FROM month WHERE id_dates='$month_counter'";
			$x2=mysqli_query($connection,$monthtable);
			$row2 = mysqli_fetch_assoc($x2);
			foreach (array_keys($row2) as $key2){if ($count2 == 2){$month=$row2[$key2];$count2=0;};$count2=$count2+1;};
			$counter1=0;
			while($counter1 <= 30){
			$protocolstable="SELECT * FROM calendar WHERE event_id='$counter1'";
			...
			foreach (array_keys($row) as $key){
				...
				$count=$count+1;
				if ($count == 2 && $row['month']==$month){echo "<tr><th scope='row'>";echo $row[$key]; echo "</th>";};
				if ($count == 3 && $row['month']==$month){echo "<td>";echo $row[$key]; echo "</td>";};
				if ($count == 4 && $row['month']==$month){echo "<td>";echo $row[$key]; echo "</td>";};  
				if ($count == 5 && $row['month']==$month){echo "<td>";echo $row[$key]; echo "</td>";};  
				if ($count == 6 && $row['month']==$month){echo "<td>";echo $row[$key]; echo "</td>";};   
				if ($count == 7 && $row['month']==$month){echo "<td>";echo $row[$key]; echo "</td></tr>"; $count=0;};
				if ($count == 20){$counter1=$counter1+1;};
